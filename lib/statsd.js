...
const Client = function (host, port, prefix, suffix, globalize, cacheDns, mock,
  ...
  this.prefix = expandPrefix(options.prefix, options.prefixSeparator);
  this.suffix = expandSuffix(options.suffix, options.suffixSeparator);
...
const ChildClient = function (parent, options) {
  ...
  Client.call(this, {
    ...
    /*
     * Turns out this is... complicated, because of the auto-prefixing behaviour
     */


    prefix      : (options.prefix || '') + parent.prefix, // Child has its prefix prepended to parent's prefix
    prefixSeparator: (options.prefix ? options.prefixSeparator : ''),
    suffix      : parent.suffix + (options.suffix || ''), // Child has its suffix appended to parent's suffix
    suffixSeparator: (options.suffix ? options.prefixSeparator: ''),
...
/**
 * Compute the full prefix, using the given prefix (defaulting to ""), and
 * given separator (defaulting to "."). If the separator appears in given
 * prefix, it is not duplicated.
 */
function expandPrefix(prefix, separator) {
  prefix = prefix || '';
...
/**
 * Compute the full suffix, using the given suffix (defaulting to ""), and
 * given separator (defaulting to "."). If the separator appears in given
 * suffix, it is not duplicated.
 */
function expandSuffix(suffix, separator) {
  suffix = suffix || '';
  ...
  if (suffix.length === 0 || separator.length === 0) {
    return suffix;
  }
