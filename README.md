...
* `prefix`:      What to prefix each stat name with `default: ''`. A period (`.`) delimiter is added automatically if needed.
* `suffix`:      What to suffix each stat name with `default: ''`. A period (`.`) delimiter is added automatically if needed.
